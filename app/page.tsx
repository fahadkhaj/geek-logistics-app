...
import { 
  ...
  CreditCard, Mail, PhoneCall, Zap, Navigation, 
...
export default function LandingPage() {
  ...
  // HYDRATION SHIELD: Prevents mismatch on initial server render
  if (!mounted) return <div className="min-h-screen bg-[#f8fafc]" />;

  const t = translations[lang];
  
  // PREMIUM DARK & LIGHT MODE PALETTES
  const bgMainClass = isDark ? 'bg-[#0B0F19]' : 'bg-[#f8fafc]'; // Deeper, richer dark background
  const textPrimary = isDark ? 'text-white' : 'text-slate-900';
  const textSecondary = isDark ? 'text-slate-300' : 'text-slate-500'; // Softer, more readable secondary text in dark mode
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-indigo-500/30 overflow-x-hidden transition-colors duration-700 ${bgMainClass}`}>
      
      {/* PREMIUM GLOWING BACKGROUND FX */}
      ...
      <main className="relative z-10 flex flex-col items-center justify-center pt-32 sm:pt-40 pb-16 sm:pb-20 px-4 sm:px-6 lg:px-8">
        
        {/* HERO SECTION */}
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="text-center max-w-4xl mx-auto w-full">
        ...
        {/* FULLY RESPONSIVE DASHBOARD MOCKUP */}
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4, ease: transitionEase }} className="w-full max-w-6xl mx-auto mt-16 sm:mt-20 mb-24 sm:mb-40 relative">
          
          <div className={`rounded-3xl sm:rounded-[2.5rem] border overflow-hidden flex flex-col md:flex-row min-h-[450px] md:h-[600px] ${glassCard}`}>
            
            {/* Mockup Sidebar (Hidden on Mobile) */}
            <div className={`hidden md:flex flex-col items-center w-24 py-8 border-r ${isDark ? 'border-white/10 bg-[#080b1a]/40' : 'border-slate-200 bg-white/50'}`}>
            ...
            <div className="flex-1 flex flex-col p-4 sm:p-6 lg:p-8 overflow-hidden">
               ...
               {/* Mock Stats Row (Scrollable on mobile) */}
               <div className="flex sm:grid sm:grid-cols-3 gap-3 sm:gap-4 mb-6 overflow-x-auto pb-2 sm:pb-0 hide-scrollbar">
                 {[
               ...
               {/* Mock Map & Activity Split */}
               <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-[250px] sm:min-h-0">
                 
                 {/* Map Area */}
                 <div className={`lg:col-span-2 h-[250px] lg:h-auto rounded-2xl border relative overflow-hidden flex items-center justify-center ${innerCard}`}>
                   ...
                   {/* Mobile Map Overlay (Instead of floating badge) */}
                   <div className="absolute top-4 right-4 lg:hidden flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/90 dark:bg-black/80 backdrop-blur-md border border-slate-200 dark:border-white/10 shadow-sm">
          ...
          {/* Floating Outer UI Badges (Hidden on mobile to prevent squishing) */}
        ...
        {/* CLARIFIED "HOW IT WORKS" SECTION */}
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className="max-w-7xl mx-auto w-full mb-24 sm:mb-40 text-center">
        ...
        {/* FEATURES SECTION */}
        <div className="max-w-7xl w-full mx-auto space-y-24 sm:space-y-32">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className="text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 text-left">
              {[
                ...
                <motion.div key={i} variants={fadeUp} className={`p-6 sm:p-8 rounded-3xl border transition-all hover:-translate-y-2 hover:shadow-2xl ${glassCard}`}>
          ...
          {/* STATS STRIP */}
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className={`p-8 sm:p-16 rounded-[2rem] sm:rounded-[3rem] border ${glassCard}`}>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 sm:gap-0 divide-y sm:divide-y-0 sm:divide-x divide-inherit text-center">
              {[
                ...
                <motion.div key={i} variants={fadeUp} className="flex flex-col items-center justify-center pt-6 sm:pt-0 first:pt-0">
          ...
          {/* FINAL CTA */}
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className="text-center max-w-3xl mx-auto pb-8 sm:pb-16">
      ...
      <footer className={`relative z-10 border-t transition-colors ${isDark ? 'border-white/10 bg-[#080b1a]' : 'border-slate-200 bg-white'}`}>
